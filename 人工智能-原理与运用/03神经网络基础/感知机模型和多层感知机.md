...
#### 激活函数
通常使用[[激活函数#阶跃函数|阶跃函数]]或符号函数，将加权和转换为输出
## 感知机模型数学表达
...
#### 非线性激活函数
单层感知机使用的是线性激活函数，这样的激活函数过与简单，不能捕捉复杂的模式。而多层感知机引入了非线性激活函数（如[[激活函数#Sigmoid 函数|Sigmoid]]，[[激活函数#Tanh 函数|Tanh]],[[激活函数#ReLU 函数|ReLU]]等 ），这写函数引入非线性使网络能够学习和表达复杂的关系
#### 反向传播算法
...
# 转换为 TensorX_train = Tensor(X_train)  
